...
export async function renderArtifact(input: {
  ...
  if (input.format === "pptx") {
    ...
    return { format: "pptx", mimeType: ARTIFACT_MIME.pptx, filename: presentationFileName(model, input.version), bytes, unitCount: model.slides.length, unitLabel: "slides" };
  }
  ...
  const html = renderHtmlDashboard(model, input.sources);
...
export async function validateArtifact(artifact: RenderedArtifact) {
